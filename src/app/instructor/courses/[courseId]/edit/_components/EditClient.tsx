...
export const EditClient = ({ course }: EditClientProps) => {
  ...
  const onVideoUploadComplete = async (url: string) => {
    try {
      ...
      alert(
        "Upload successful but failed to save to course. Please try again.",
      );
    }
  };

  const onMaterialUploadComplete = async (
    url: string,
    title: string,
    size: number,
    type: string,
  ...
  const createExercise = async () => {
    try {
      const response = await axios.post(
        `/api/courses/${course.id}/exercises`,
        exerciseForm,
      );
  ...
  const connectRepo = async () => {
    ...
    try {
      const response = await axios.post(
        `/api/courses/${course.id}/code-repos`,
        repoForm,
      );
                        ...
                        <div className="flex items-center gap-x-4">
                          ...
                          {editingVideoId === video.id ? (
                            <div className="flex items-center gap-2">
                              ...
                              />
                              <button
                                onClick={() => updateVideoTitle(video.id)}
                                className="text-neon-green hover:scale-110 transition-transform"
                              >
          ...
          {activeTab === "materials" && (
            ...
            >
              <div className="glass-card p-6">
                ...
                <MaterialUpload
                  courseId={course.id}
                  onUploadComplete={onMaterialUploadComplete}
                />
              </div>
              ...
              <div className="glass-card p-6">
                ...
                <div className="mt-4">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setRepoForm({ ...repoForm, repoUrl: e.target.value })
                    }
                ...
                <div className="mt-4">
                  ...
                  <textarea
                    ...
                    onChange={(e) =>
                      setRepoForm({ ...repoForm, description: e.target.value })
                    }
              ...
              <div className="glass-card p-6">
                ...
                <div className="space-y-3">
                  ...
                  {materials.map((material: any) => (
                    <div
                      key={material.id}
                      className="group flex items-center justify-between p-4 bg-white/5 rounded-2xl border border-white/10 hover:border-primary/50 transition-all"
                    >
                      <div className="flex items-center gap-x-4">
            ...
            >
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          {["A", "B", "C", "D"].map((key) => (
                            ...
                            >
                              <div
                                className={`w-8 h-8 rounded-lg flex items-center justify-center font-bold text-xs cursor-pointer transition-all ${exerciseForm.correctAnswer === key ? "bg-primary text-black" : "bg-white/5 text-gray-400"}`}
                                onClick={() =>
                                  setExerciseForm({
                                    ...exerciseForm,
                                    correctAnswer: key,
              ...
              <div className="space-y-3">
                ...
                {exercises.map((exercise: any) => (
                  <div
                    key={exercise.id}
                    className="glass-card p-5 group hover:border-primary/50 transition-all"
                  >
