  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (ext === "pdf") materialType = "PDF";
        ...
        ["zip", "rar", "js", "ts", "py", "html", "css"].includes(ext || "")
      )
        materialType = "CODE";
      ...
      setSuccess(true);
      onUploadComplete(data.publicUrl, file.name, file.size, materialType);
    } catch (err: any) {
      console.error(
      ...
      setError(
        typeof msg === "string" ? msg : "Something went wrong during upload",
      );
    } finally {
      setUploading(false);
    }
